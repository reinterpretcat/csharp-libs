...
namespace UtyRx.Tests.Observable
{
    ...
    public class ObservableTest
    {
        ...
        {
            var m = UtyRx.Observable.Empty<int>().Materialize().ToArrayWait();
            m[0].Kind.Is(NotificationKind.OnCompleted);

            m = UtyRx.Observable.Range(1, 3).Materialize().ToArrayWait();
            m[0].Value.Is(1);
            m[1].Value.Is(2);
            m[2].Value.Is(3);
            m[3].Kind.Is(NotificationKind.OnCompleted);

            m = UtyRx.Observable.Range(1, 3).Concat(UtyRx.Observable.Throw<int>(new Exception())).Materialize().ToArrayWait();
            m[0].Value.Is(1);
            m[1].Value.Is(2);
            m[2].Value.Is(3);
        ...
        {
            var m = UtyRx.Observable.Empty<int>().Materialize().Dematerialize().ToArrayWait();
            m.IsCollection();

            m = UtyRx.Observable.Range(1, 3).Materialize().Dematerialize().ToArrayWait();
            m.IsCollection(1, 2, 3);

            var l = new List<int>();
            UtyRx.Observable.Range(1, 3).Concat(UtyRx.Observable.Throw<int>(new Exception())).Materialize()
                .Dematerialize()
        ...
        {
            UtyRx.Observable.Range(1, 3).DefaultIfEmpty(-1).ToArrayWait().IsCollection(1, 2, 3);
            UtyRx.Observable.Empty<int>().DefaultIfEmpty(-1).ToArrayWait().IsCollection(-1);
        }

        [Test]
        public void IgnoreElements()
        {
            var xs = UtyRx.Observable.Range(1, 10).IgnoreElements().Materialize().ToArrayWait();
        ...
        public void ForEachAsync()
        {
            {
                var list = new List<int>();
                var xs = UtyRx.Observable.Range(1, 10).ForEachAsync(x => list.Add(x)).ToArray().Wait();
...

            {
                ...
                var xs = UtyRx.Observable.Range(1, 10).ForEachAsync((x, i) =>
                {
                    list.Add(x);
                    listI.Add(i);
                }).ToArray().Wait();
