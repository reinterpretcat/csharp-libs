    ...
    public class ObservableTimeTest
    {
        ...
        public void TimerTest()
        {
            // periodic(Observable.Interval)
            {
                ...
                var xs = UtyRx.Observable.Timer(TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(1))
                    .Take(3)
                    .Timestamp()
                    .ToArray()
                    .Wait();
            ...
            {
                var now = Scheduler.ThreadPool.Now;
                var xs = UtyRx.Observable.Timer(TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(1))
                    .Take(3)
                    .Timestamp()
            ...
            {
                var now = Scheduler.ThreadPool.Now;
                var xs = UtyRx.Observable.Timer(now.AddSeconds(2), TimeSpan.FromSeconds(1))
                    .Take(3)
                    .Timestamp()
                    .Select(x => Math.Round((x.Timestamp - now).TotalSeconds, 0))
                    .ToArray()
            ...
            {
                var now = Scheduler.ThreadPool.Now;
                var xs = UtyRx.Observable.Timer(TimeSpan.FromSeconds(2))
                    .Timestamp()
                    .Select(x => Math.Round((x.Timestamp - now).TotalSeconds, 0))
                    .ToArray()
                    .Wait();
            ...
            // non periodic scheduler
            {
                ...
                var xs = UtyRx.Observable.Timer(TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(1), Scheduler.CurrentThread)
                    .Take(3)
                    .Timestamp()
                    .Select(x => Math.Round((x.Timestamp - now).TotalSeconds, 0))
                    .ToArray()
        ...
        public void DelayTest()
        {
            ...
            var xs = UtyRx.Observable.Range(1, 3)
        ...
        {
            // 2400, 4800, 7200, 9600
            var xs = UtyRx.Observable.Timer(TimeSpan.Zero, TimeSpan.FromSeconds(1))
                .Take(10)
        ...
        {
            var xs = UtyRx.Observable.Concat(
                    UtyRx.Observable.Return(1).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(5).Delay(TimeSpan.FromSeconds(2)),
                    UtyRx.Observable.Return(9).Delay(TimeSpan.FromSeconds(3))
                )
                .Timestamp()
        ...
        public void ThrottleTest()
        {
            var xs = UtyRx.Observable.Concat(
                    UtyRx.Observable.Return(1).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(2).Delay(TimeSpan.FromSeconds(2)),
                    UtyRx.Observable.Return(3).Delay(TimeSpan.FromSeconds(2)),
                    UtyRx.Observable.Return(4).Delay(TimeSpan.FromSeconds(2)),
                    UtyRx.Observable.Return(5).Delay(TimeSpan.FromSeconds(2)),
                    UtyRx.Observable.Return(6).Delay(TimeSpan.FromSeconds(3)), // over 2500
                    UtyRx.Observable.Return(7).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(8).Delay(TimeSpan.FromSeconds(1)) // with onCompleted
                )
                .Timestamp()
        ...
        {
            var xs = UtyRx.Observable.Concat(
                    UtyRx.Observable.Return(1),
                    UtyRx.Observable.Return(2).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(3).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(4).Delay(TimeSpan.FromSeconds(0.4)),
                    UtyRx.Observable.Return(5).Delay(TimeSpan.FromSeconds(0.2)), // over 2500
                    UtyRx.Observable.Return(6).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(7).Delay(TimeSpan.FromSeconds(1)),
                    UtyRx.Observable.Return(8).Delay(TimeSpan.FromSeconds(1)), // over 2500
                    UtyRx.Observable.Return(9) // withCompleted
                )
                .Timestamp()
