...
namespace UtyRx.Tests.Observable
{
    ...
    public class ObservableConcatTest
    {
        ...
        public void Concat()
        {
            var a = UtyRx.Observable.Range(1, 5, Scheduler.ThreadPool);
            var b = UtyRx.Observable.Range(10, 3, Scheduler.ThreadPool);
            var c = UtyRx.Observable.Return(300, Scheduler.ThreadPool);

            UtyRx.Observable.Concat(a, b, c).ToArray().Wait().IsCollection(1, 2, 3, 4, 5, 10, 11, 12, 300);
        }
        ...
        public void ZipMulti()
        {
            ...
            var l = Enumerable.Empty<Unit>().Select(_ => Notification.CreateOnNext(new { x = 0, y = 0 })).ToList();
            UtyRx.Observable.Zip(a, b).Select(xs => new { x = xs[0], y = xs[1] }).Materialize().Subscribe(x => l.Add(x));
        ...
        public void ZipNth()
        {
            ...
            UtyRx.Observable.Zip(a, b, c, (x, y, z) => new { x, y, z }).Materialize().Subscribe(x => l.Add(x));
        ...
        public void WhenAll()
        {
            ...
            var l = Enumerable.Empty<Unit>().Select(_ => Notification.CreateOnNext(new { x = 0, y = 0 })).ToList();
            UtyRx.Observable.WhenAll(a, b).Select(xs => new { x = xs[0], y = xs[1] }).Materialize().Subscribe(x => l.Add(x));
        ...
        public void CombineLatestMulti()
        {
            ...
            var l = Enumerable.Empty<Unit>().Select(_ => Notification.CreateOnNext(new { x = 0, y = 0 })).ToList();
            UtyRx.Observable.CombineLatest(a, b).Select((xs) => new { x = xs[0], y = xs[1] }).Materialize().Subscribe(x => l.Add(x));
        ...
        public void ZipLatestMulti()
        {
            ...
            var l = Enumerable.Empty<Unit>().Select(_ => Notification.CreateOnNext(new { x = 0, y = 0 })).ToList();
            UtyRx.Observable.ZipLatest(a, b).Select((xs) => new { x = xs[0], y = xs[1] }).Materialize().Subscribe(x => l.Add(x));
        ...
        {
            var seq = UtyRx.Observable.Range(1, 5);
        ...
        public void Merge()
        {
            ...
            var complete = false;
            UtyRx.Observable.Merge(s1, s2, s3).Subscribe(list.Add, () => complete = true);
        ...
        public void MergeConcurrent()
        {
            ...
            var list = new List<int>();
            var complete = false;
            UtyRx.Observable.Merge(new[] { s1, s2, s3 }, maxConcurrent: 2).Subscribe(list.Add, () => complete = true);
