...
namespace UtyRx.Tests.Observable
{
    ...
    public class ObservablePagingTest
    {
        ...
        public void Buffer()
        {
            var xs = UtyRx.Observable.Range(1, 10)
                .Buffer(3)
                .ToArray()
                .Wait();
        ...
        {
            var hoge = UtyRx.Observable.Return(1000).Delay(TimeSpan.FromSeconds(4));
        ...
        {
            var xs = UtyRx.Observable.Return(10).Delay(TimeSpan.FromMilliseconds(3500))
        ...
        public void BufferTimeComplete()
        {
            ...
            var xs = UtyRx.Observable.Return(1).Delay(TimeSpan.FromSeconds(2))
                .Concat(UtyRx.Observable.Return(1).Delay(TimeSpan.FromSeconds(2)).Skip(1))
                .Buffer(TimeSpan.FromSeconds(3))
                .ToArray()
                .Wait();
        ...
        {
            var xs = UtyRx.Observable.Empty<int>()
        ...
        {
            var xs = UtyRx.Observable.Range(1, 2)
                .Buffer(2)
                .ToArray()
                .Wait();
        ...
        public void BufferSkip()
        {
            // count > skip
            {
                var xs = UtyRx.Observable.Range(1, 10).Buffer(3, 1)
                    .ToArray()
                    .Wait();
            ...
            // count == skip
            {
                var xs = UtyRx.Observable.Range(1, 10).Buffer(3, 3)
                    .ToArray()
                    .Wait();
            ...
            // count < skip
            {
                var xs = UtyRx.Observable.Range(1, 20).Buffer(3, 5)
                    .ToArray()
                    .Wait();
        ...
        public void BufferTimeAndCount()
        {
            // time...count...complete
            {
                var xs = UtyRx.Observable.Return(1000)
                    .Concat(UtyRx.Observable.Return(99).Delay(TimeSpan.FromMilliseconds(1500)))
                    .Concat(UtyRx.Observable.Range(1, 7))
            ...
            // time...count...time
            {
                var xs = UtyRx.Observable.Return(1000)
                    .Concat(UtyRx.Observable.Return(99).Delay(TimeSpan.FromMilliseconds(1500)))
                    .Concat(UtyRx.Observable.Range(1, 7))
                    .Concat(UtyRx.Observable.Never<int>())
                    .Buffer(TimeSpan.FromSeconds(1), 5)
                    .Take(3)
                    .ToArray()
                    .Wait();
            ...
            // time(before is canceled)
            {
                ...
                var result = UtyRx.Observable.Return(10).Delay(TimeSpan.FromSeconds(2))
                    .Concat(UtyRx.Observable.Range(1, 2))
                    .Concat(UtyRx.Observable.Return(1000).Delay(TimeSpan.FromSeconds(2)))
                    .Concat(UtyRx.Observable.Never<int>())
        ...
        {
            {
                ...
                var timer = UtyRx.Observable.Timer(TimeSpan.FromSeconds(0), TimeSpan.FromMilliseconds(10))
                    .Timestamp();
        ...
        {
            var xs = UtyRx.Observable.Range(1, 5).Pairwise((x, y) => x + ":" + y).ToArrayWait();
        ...
        {
            var xs = UtyRx.Observable.Range(1, 5).Pairwise().ToArrayWait();
        ...
        {
            var record = UtyRx.Observable.Range(1, 2).TakeLast(3).Record();
            record.Values.IsCollection(1, 2);

            record = UtyRx.Observable.Range(1, 3).TakeLast(3).Record();
            record.Values.IsCollection(1, 2, 3);

            record = UtyRx.Observable.Range(1, 4).TakeLast(3).Record();
            record.Values.IsCollection(2, 3, 4);

            record = UtyRx.Observable.Range(1, 10).TakeLast(3).Record();
            record.Values.IsCollection(8, 9, 10);

            record = UtyRx.Observable.Empty<int>().TakeLast(3).Record();
        ...
        public void TakeLastDuration()
        {
            ...
            // 0, 200, 400, 600, 800
            var data = UtyRx.Observable.Timer(TimeSpan.Zero, TimeSpan.FromMilliseconds(200))
                .Take(5)
