...
namespace UtyRx.Tests.Observable
{
    ...
    public class ObservableEventsTest
    {
        ...
        public void FromEvent()
        {
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<EventHandler, EventArgs>(
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<EventHandler<MyEventArgs>, MyEventArgs>(
                    h => (sender, e) => h.Invoke(e),
                    h => test.Event2 += h, h => test.Event2 -= h)
                    .Subscribe(x => isRaised = true);
                test.Fire(2);
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<MyEventHandler, MyEventArgs>(
                    h => (sender, e) => h.Invoke(e),
                    h => test.Event3 += h, h => test.Event3 -= h)
                    .Subscribe(x => isRaised = true);
                test.Fire(3);
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<Action, Unit>(
                    h => () => h(Unit.Default),
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent(
                    h => test.Event4 += h, h => test.Event4 -= h)
                    .Subscribe(x => isRaised = true);
                test.Fire(4);
                isRaised.IsTrue();
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<Action<int>, int>(
                    h => h,
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<int>(
                    h => test.Event5 += h, h => test.Event5 -= h)
                    .Subscribe(x => isRaised = true);
                test.Fire(5);
                isRaised.IsTrue();
            ...
            {
                var isRaised = false;
                var d = UtyRx.Observable.FromEvent<Action<int, string>, Tuple<int, string>>(
