...
namespace UtyRx.Tests.InternalUtil
{
    class DecrementEnumerator : IEnumerator
    {
        ...
        {
            return String.Format("{0}/{1}", _count, _original);
        }
    }
    ...
    {
        static UtyRx.InternalUtil.MicroCoroutine Create()
        {
            return new UtyRx.InternalUtil.MicroCoroutine(ex => Console.WriteLine(ex));
        }

        static int FindLast(UtyRx.InternalUtil.MicroCoroutine mc)
        {
        ...
        static int GetTailDynamic(UtyRx.InternalUtil.MicroCoroutine mc)
        {
